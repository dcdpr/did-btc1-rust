...
use crate::transformation::JcsTransformation;
...
impl CryptoSuite for Bip340JcsSuite {
    ...
    fn create_proof(&self, document: &Document, options: &ProofOptions) -> Result<Document> {
        ...
        if let Some(context) = document.get_context() {
            proof_options.insert("@context".to_string(), Value::Array(context));
        }
    ...
    fn configure_proof(&self, document: &Document, options: &ProofOptions) -> Result<Vec<u8>> {
        // Clone proof options
        let mut proof_config = options.options.clone();
    ...
    fn verify(&self, hash_data: &[u8], proof_bytes: &[u8], proof: &Proof) -> Result<bool> {
        // Get verification method
        ...
        // Verify signature
        bip340_verify(&hash_array, &signature, &public_key)
    }
}
